function V2_GUI_Zeitkonstante_Spule(Zeit,Kanal_A,Kanal_B,Infos)
    ...
    set(Achse_2,'Units','normalized',...
        ...
        'Box','on','XGrid','on','YGrid','on','XLim',[min(Zeit) max(Zeit)]);
    set(get(Achse_2,'YLabel'),'String','current i [A]');
    Achse_2.NextPlot = 'add';

    Linie_iA_roh = line(Zeit,Kanal_B,'Parent',Achse_2,'Color',[0.7 0.7 0.7]);
    ...
    function calc_glMW(source,callbackdata)
        ...
        if Wert < 1
            Wert = 1;
        elseif Wert > round(length(Kanal_B)/10,0);
            Wert = round(length(Kanal_B)/10,0)
        end
        UI_glMW_edit.String = num2str(Wert,'%5.0f');
        Kanal_B_temp = movmean(Kanal_B,Wert);
        Linie_iA.YData = Kanal_B_temp;
    end
