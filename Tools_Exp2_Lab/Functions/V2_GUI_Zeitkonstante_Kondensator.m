function V2_GUI_Zeitkonstante_Kondensator(Zeit,Spannung_Vers,Spannung_Kond,Infos)
    ...
    set(get(Achse_1,'YLabel'),'String','supply voltage U [V]');
    ...
    set(Achse_2,'Units','normalized',...
        ...
        'Box','on','XGrid','on','YGrid','on','XLim',[min(Zeit) max(Zeit)]);
    set(get(Achse_2,'YLabel'),'String','capacitor voltage U_C [V]');
    Achse_2.NextPlot = 'add';

    Linie_U2_roh = line(Zeit,Spannung_Kond,'Parent',Achse_2,'Color',[0.7 0.7 0.7]);
    ...
    UI_glMW_text = uicontrol('style','text','Units','normalized',...
        'Position',[0.1 Pos_unten 0.3 0.03],'Visible','on',...
        'String','number of values for calculating moving average'); 
    
    UI_glMW_edit = uicontrol('style','edit','Units','normalized',...
        'Position',[0.41 Pos_unten 0.1 0.05],'Visible','on',...
        'String','1','callback',@calc_glMW);  
    ...
    UI_U1_MW_text = uicontrol('style','text','Units','normalized',...
        ...
        'HorizontalAlignment','left','String','supply voltage [V]','Visible','off');
    UI_U2_MW_text = uicontrol('style','text','Units','normalized',...
        'Position',[0.6 Pos_unten+(1-Pos_unten)/3 0.25 (1-Pos_unten)/3*0.9],...
        'HorizontalAlignment','left','String','capacitor voltage [V]','Visible','off');
    ...
    function calc_glMW(source,callbackdata)
        String = UI_glMW_edit.String;
        String = strrep(String,',','.');
        Wert = str2double(String);
        Wert = round(Wert,0);
        if Wert < 1
            Wert = 1;
        elseif Wert > round(length(Spannung_Kond)/10,0);
            Wert = round(length(Spannung_Kond)/10,0)
        end
        UI_glMW_edit.String = num2str(Wert,'%5.0f');
        Spannung_Kond_temp = movmean(Spannung_Kond,Wert);
        Linie_U2.YData = Spannung_Kond_temp;
    end
